...
本文档说明如何在 **Qwen3 基座 + LoRA 微调** 下，用项目内脚本完成数据处理与训练，并统一使用 **`task-1.sh` ~ `task-4.sh`** 与 **环境变量** 控制配置，无需手改 cnfg 文件。
...
1. **入口与配置**
   - 打开 **`task-1.sh` ~ `task-4.sh`**：看它设置了哪些环境变量、最后执行哪条命令。
...
| 路径 | 说明 |
|------|------|
| `task-1.sh` ~ `task-4.sh` | 激活 conda、设置 `PRE_TRAINED_M` / `DATA_ID` / `LORA_RANK` / `LORA_ALPHA`（`task-4.sh` 还支持 `USE_AMP`），执行 `python adv/train/plm/train_lora_qwen.py`。 |
| `scripts/plm/llm/pubdatasets_to_srctgt.py` | 从 pubdatasets（NQ/SQuAD/MetaMathQA/ToolBench）生成 src/tgt 文本；NQ 支持 `--workers` 并行。 |
...
├── task-1.sh                                  # SQuAD closed
├── task-2.sh                                  # ToolBench
├── task-3.sh                                  # NQ closed
├── task-4.sh                                  # MetaMathQA
...
│
└── scripts/plm/llm/
    └── pubdatasets_to_srctgt.py               # 从 pubdatasets 目录生成各数据集的 src/tgt 文本对（NQ/SQuAD/MetaMathQA/ToolBench）
```
...
| 层级 | 作用 |
|------|------|
| **入口与数据** | `task-1.sh`~`task-4.sh` 设好环境变量后执行 `train_lora_qwen.py`；数据来自 `cache/llm/pubdatasets_*/` 下的 train.h5、dev.h5，由 `pubdatasets_to_srctgt` → map → sort → `mkiodata_llmdec` 生成。 |
...
NQ、SQuAD、MetaMathQA、ToolBench 形态不一（问答、推理、多轮对话等），但训练目标统一为：**在「指令 + 回答」的序列上，只对「回答」这一段算 loss**，这样模型学会的是“见指令后生成答案”，而不是去拟合指令里的字。因此需要：

1. **统一成「指令 → 回答」两条文本**  
   四个数据集的原始字段不同（MetaMathQA 的 query/response、SQuAD 的 question/answers、NQ 的 question/short_answers 或 yes/no、ToolBench 的 conversations），第一步要把它们都变成「一条指令（src）、一条回答（tgt）」的文本对，便于后面用同一套 tokenizer 和 HDF5 格式。
...
|------|------|----------------|-----------------|
| **1. 原始数据 → src/tgt 文本** | 四个数据集统一成「指令一行、回答一行」 | **`scripts/plm/llm/pubdatasets_to_srctgt.py`** | `python scripts/plm/llm/pubdatasets_to_srctgt.py --pubdatasets <pubdatasets根目录> --out cache/llm`；单数据集加 `--dataset metamathqa` / `nq` / `squad` / `toolbench`。输出在 **`cache/llm/pubdatasets_<名>/`**，得到 `src.train.txt`、`tgt.train.txt`、`src.dev.txt`、`tgt.dev.txt`。 |
...
| **5. 训练时读数据** | 按 data_id 找 train/dev.h5 | **`cnfg/base.py`**（data_id、cache_dir）、**`adv/train/plm/train_lora_qwen.py`**（环境变量 DATA_ID 覆盖）、**`utils/train/llm.py`**（PMaskDataConverter） | 在 **`task-1.sh`~`task-4.sh`** 中设 `DATA_ID=llm/pubdatasets_metamathqa`（或 squad_closed / nq_closed / toolbench）；训练脚本会读 **`cache_dir + DATA_ID + "/train.h5"`** 与 **`dev.h5`**。 |

**建议路径小结**（以 MetaMathQA 为例，其他数据集只改目录名）：

- **工作目录（每个数据集一个）**：`cache/llm/pubdatasets_metamathqa/`（或 `pubdatasets_nq_closed`、`pubdatasets_squad_closed`、`pubdatasets_toolbench`）。
- **你主要要改/要设的**：环境变量 **`TOKENIZER`**（Qwen3 权重目录）、**`WKD`**（当前数据集目录）、**`DATA_ID`**（训练时用，如 `llm/pubdatasets_metamathqa`）；**`task-1.sh`~`task-4.sh`** 里的 **`PRE_TRAINED_M`**、**`DATA_ID`**（或运行时用环境变量覆盖）。
...
| 数据集 | 训练 | 验证/测试 | 字段说明 |
|--------|------|-----------|----------|
| MetaMathQA | `metamathqa/train/data-*.arrow`（`load_from_disk`） | 通常无官方 dev（脚本会自动切分） | `query` → 指令，`response` → 回答 |
| SQuAD | `squad/plain_text/train-*.parquet` | `squad/plain_text/validation-*.parquet` | 闭卷设定：`question` → 指令，`answers.text[0]` → 回答 |
| NQ | `natural_questions/default/train-*-of-00287.parquet` | `natural_questions/default/validation-*`（或 `dev/validation-*`） | 闭卷设定：`question.text` → 指令，短答或 `YES/NO` → 回答 |
...
- 单数据集：`--dataset metamathqa|nq|squad|toolbench`
- 限制样本：`--max-train 5000 --max-dev 500`
- NQ 加速：`--workers 8`

输出：`cache/llm/pubdatasets_metamathqa/`、`pubdatasets_squad_closed/`、`pubdatasets_nq_closed/`、`pubdatasets_toolbench/` 等目录，每目录含 `src.train.txt`、`tgt.train.txt`、`src.dev.txt`、`tgt.dev.txt`。
...
export WKD=cache/llm/pubdatasets_metamathqa
...
### 3.6 步骤 5：用 `task-1.sh`~`task-4.sh` 启动训练（推荐）

四个数据集都做完 3.2～3.5 后，按数据集使用对应 `task-*.sh` 启动训练，无需改 cnfg 文件。训练脚本 **`adv/train/plm/train_lora_qwen.py`** 会在开头根据环境变量覆盖：`DATA_ID`、`PRE_TRAINED_M`、`LORA_RANK`、`LORA_ALPHA`。

**前置**：基座须为项目可读的 **model.h5**（或 .bin），路径设为 `PRE_TRAINED_M`。

1. **修改对应 `task-*.sh` 中的默认值（或运行时用环境变量覆盖）**
...
3. **执行**
   ...
   或显式传参：
   ```bash
   DATA_ID=llm/pubdatasets_metamathqa PRE_TRAINED_M=/path/to/Qwen3-0.6B/model.h5 bash task-4.sh
   ```
...
python scripts/plm/llm/pubdatasets_to_srctgt.py --pubdatasets /home/gpchen/pubdatasets --out cache/llm --dataset metamathqa
python tools/plm/map/qwen/v3.py $WKD/src.train.txt $TOKENIZER $WKD/src.train.txt.ids instruct_auto
python tools/plm/map/qwen/v3.py $WKD/tgt.train.txt $TOKENIZER $WKD/tgt.train.txt.ids assistant
python tools/plm/map/qwen/v3.py $WKD/src.dev.txt   $TOKENIZER $WKD/src.dev.txt.ids   instruct_auto
python tools/plm/map/qwen/v3.py $WKD/tgt.dev.txt   $TOKENIZER $WKD/tgt.dev.txt.ids   assistant
...
# 启动训练（任选其一）
bash task-4.sh
# 或：DATA_ID=llm/pubdatasets_metamathqa PRE_TRAINED_M=/path/to/Qwen3-0.6B/model.h5 python adv/train/plm/train_lora_qwen.py
```
...
**本仓库中不包含任何预训练基座权重**（没有 .h5 / .bin 文件）。`cnfg/plm/qwen/v3/base.py` 与 `task-1.sh`~`task-4.sh` 里的权重路径均需按本机实际路径设置。
...
| **实验室/组内** | 脚本中常见路径 `/home/common/plm/Qwen/Qwen3-8B`，若你所在环境有共享的 Qwen 目录或**已转好的 model.h5**，可直接向导师/同门索取路径，并在 `task-*.sh` 或 cnfg 中设为 `PRE_TRAINED_M` / `pre_trained_m`。 |
...
**方式 A（推荐）：环境变量，配合 `task-1.sh`~`task-4.sh`**
...
- 打开 **`cnfg/plm/qwen/v3/base.py`**，找到：
  ...
  ```
- 保存后运行 `python adv/train/plm/train_lora_qwen.py` 或 `bash task-4.sh`（此时可不再设 `PRE_TRAINED_M`）。
...
- **推荐**：不改 cnfg，用 **`task-1.sh`~`task-4.sh`** 设置 **`PRE_TRAINED_M`**（及 `DATA_ID`、`LORA_RANK`、`LORA_ALPHA` 等），运行 `adv/train/plm/train_lora_qwen.py`。
...
- **推荐使用方式**：完成 3.2～3.5 后，在对应 **`task-*.sh`** 中设好 `PRE_TRAINED_M` 与 `DATA_ID`，执行脚本即可复现 Qwen3 + LoRA 训练。
