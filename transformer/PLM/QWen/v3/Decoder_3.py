...
本文件定义（LORA_LAYER_SELECTION_GUIDE §11 方案 C）：
- DecoderLayer：先 `ResSelfAttn`，再 `ResHPLSTM`，再 `ff`（无并行 `HPLSTM` 支路）。
- 训练时解冻 `reslstm`，勿对不存在的 `hplstm` 调用 `unfreeze_hplstm`。
- 另见 `Decoder_1.py`（A）、`Decoder_2.py`（B）。
"""
...
class DecoderLayer(DecoderLayerBase):
	"""
	方案 C：x1 = ResSelfAttn(...)；x2 = ResHPLSTM(x1)；context = ff(x2)。（x1 已为 attention 子层输出。）
	"""
