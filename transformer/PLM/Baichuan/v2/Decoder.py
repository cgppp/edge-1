...
from modules.plm.baichuan.v2 import PositionwiseFF, ResSelfAttn
...
from cnfg.vocab.plm.baichuan.v2 import eos_id, pad_id, sos_id

class DecoderLayer(DecoderLayerBase):

	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, num_head=8, ahsize=None, norm_residual=norm_residual_default, k_rel_pos=use_k_relative_position_decoder, max_bucket_distance=relative_position_max_bucket_distance_decoder, disable_ffn_bias=disable_ffn_bias, model_name="model", **kwargs):
		...
		self.self_attn = ResSelfAttn(isize, hsize=_ahsize, num_head=num_head, dropout=attn_drop, norm_residual=norm_residual, k_rel_pos=k_rel_pos, uni_direction_reduction=True, max_bucket_distance=max_bucket_distance)
	...
	def load_plm(self, plm_parameters, model_name=None, layer_idx=None, **kwargs):
		...
		with torch_no_grad():
			copy_plm_parameter(self.self_attn.net.adaptor.weight, plm_parameters, "%s.layers.%d.self_attn.W_pack.weight" % (_model_name, layer_idx,))
			_bias_key = "%s.layers.%d.self_attn.W_pack.bias" % (_model_name, layer_idx,)
			align_linear_bias(self.self_attn.net.adaptor, plm_parameters, _bias_key)
			if self.self_attn.net.adaptor.bias is not None:
				copy_plm_parameter(self.self_attn.net.adaptor.bias, plm_parameters, _bias_key)
...
class Decoder(DecoderBase):
	...
	def __init__(self, isize, nwd, num_layer, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, emb_w=None, num_head=8, xseql=cache_len_default, ahsize=None, norm_output=True, bindemb=True, forbidden_index=None, share_layer=False, disable_pemb=disable_std_pemb_decoder, disable_ffn_bias=disable_ffn_bias, remove_classifier_bias=remove_classifier_bias, model_name="model", **kwargs):
		...
		if share_layer:
			_shared_layer = DecoderLayer(isize, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, act_drop=act_drop, num_head=num_head, ahsize=_ahsize, disable_ffn_bias=disable_ffn_bias, model_name=model_name)
			self.nets = nn.ModuleList([_shared_layer for i in range(num_layer)])
		else:
			self.nets = nn.ModuleList([DecoderLayer(isize, fhsize=_fhsize, dropout=dropout, attn_drop=attn_drop, act_drop=act_drop, num_head=num_head, ahsize=_ahsize, disable_ffn_bias=disable_ffn_bias, model_name=model_name) for i in range(num_layer)])
