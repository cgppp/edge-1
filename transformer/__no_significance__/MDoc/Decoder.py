...
class Decoder(DecoderBase):
	...
	def decode(self, inpute, src_pad_mask=None, beam_size=1, max_len=512, length_penalty=0.0, rsentind=None, fill_pad=False, **kwargs):
		...
		return self.beam_decode(inpute, src_pad_mask=src_pad_mask, beam_size=beam_size, max_len=max_len, length_penalty=length_penalty, rsentind=rsentind, fill_pad=fill_pad, **kwargs) if beam_size > 1 else self.greedy_decode(inpute, src_pad_mask=src_pad_mask, max_len=max_len, rsentind=rsentind, fill_pad=fill_pad, **kwargs)

	def greedy_decode(self, inpute, src_pad_mask=None, max_len=512, rsentind=None, fill_pad=False, sample=False, top_k=1, top_p=0.0, temp=1.0, **kwargs):
