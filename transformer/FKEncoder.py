...
from modules.base import Linear, SparseNormer
...
class EncoderLayer(nn.Module):
	...
	def __init__(self, isize, fhsize=None, dropout=0.0, attn_drop=0.0, act_drop=None, num_head=8, ahsize=None, num_anchor=None, norm_residual=norm_residual_default, enable_bias=enable_prev_ln_bias_default, enable_proj_bias=enable_proj_bias_default, sparsenorm=False, **kwargs):
		...
		_ = [GroupLinear(self.anchor_size * self.num_head, _fhsize * self.num_head, self.num_head, bias=True, trans_input=False, shuffle=False, flatten_output=False)]
		if use_glu is None:
		...
		_isize = _fhsize * self.num_head // self.anchor_size
		_ = [Linear(_isize if use_glu is None else (_isize // 2), self.hsize, bias=enable_proj_bias), Linear(self.hsize, _fhsize)]
		_drop_ind = 3
		if use_glu is None:
			_.extend([Custom_Act() if custom_act else nn.ReLU(inplace=True), Linear(_fhsize, self.hsize, bias=enable_bias)])
		else:
			use_glu = use_glu.lower()
			if use_glu == "glu":
				_.append(nn.GLU())
				...
				if _act is not None:
					...
					_drop_ind += 1
				_.append(LGLU())
			_.append(Linear(_fhsize // 2, self.hsize, bias=enable_bias))
		if _act_drop > 0.0:
			_.insert(_drop_ind, Dropout(_act_drop, inplace=inplace_after_Custom_Act))
		self.net = nn.Sequential(*_)
		self.k_adaptor = Linear(self.hsize, self.hsize, bias=enable_proj_bias)
		self.outer = Linear(self.hsize, isize, bias=enable_bias)
	...
	def forward(self, inputs, mask=None, **kwargs):
		...
		if self.attn_drop is not None:
			scores = self.attn_drop(scores)
		out = self.outer(scores.matmul(out.view(bsize, nanchor, nheads, adim).transpose(1, 2)).transpose(1, 2).contiguous().view(bsize, nquery, self.hsize))
