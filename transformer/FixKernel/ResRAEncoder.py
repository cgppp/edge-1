...
class EncoderLayer(EncoderLayerBase):
	...
	def forward(self, inputs, mask=None, **kwargs):
		...
		_res = out = scores.matmul(real_iV)
		# (bsize, nanchor, nheads * adim)
		out = self.anchor_net(out.view(bsize, nheads, -1)).view(bsize, nheads, nanchor, -1).transpose(1, 2).contiguous().view(bsize, nanchor, -1)
		out = self.net(out)
		# (bsize, nheads, nquery, nanchor)
		out = self.outer(scores.transpose(-1, -2).matmul(_res + out.view(bsize, nanchor, nheads, adim).transpose(1, 2)).transpose(1, 2).contiguous().view(bsize, nquery, self.ahsize))
