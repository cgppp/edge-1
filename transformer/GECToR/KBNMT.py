...
from transformer.GECToR.KBEncoder import Encoder
from transformer.GECToR.NMT import NMT as NMTBase
from utils.fmt.base import pad_batch
from utils.fmt.gec.kb.base import apply_op_ids
...
class NMT(NMTBase):
	...
	def forward(self, inpute, kb=None, edit=None, token_types=None, mask=None, mlm_mask=None, tgt=None, prediction=False, **kwargs):
		...
		return self.dec(self.enc(inpute, kb=kb, edit=edit, token_types=token_types, mask=_mask), mlm_mask=_mlm_mask, tgt=tgt, prediction=prediction)

	def decode(self, inpute, kb=None, beam_size=1, max_len=None, length_penalty=0.0, op_keep_bias=0.0, edit_thres=0.0, pad_id=pad_id, mask_id=mask_id, edit_pad_id=edit_pad_id, blank_id=blank_id, delete_id=delete_id, **kwargs):
		...
		for step in range(_max_len):
			if step > 0:
				...
				_mlm_mask = None
			_tag_out = self.dec(self.enc(_inpute, kb=_kb, edit=_edit, mask=_pad_mask.unsqueeze(1)), mlm_mask=_mlm_mask, tgt=None, prediction=True, op_keep_bias=op_keep_bias, edit_thres=edit_thres)[-1]
			...
			if torch_any_wodim(done_trans).item():
				...
				if step < _last_step:
					...
					break
			if step < _last_step:
				_next_i, _next_k, _next_e, _mlen = [], [], [], 0
				for _i, _k, _e, _t in zip(_inpute.tolist(), _kb.tolist(), _edit.tolist(), _tag_out.tolist()):
					_iu, _ku, _eu = apply_op_ids(_i, _k, _e, _t)
					_next_i.append(_iu)
					_next_k.append(_ku)
					_next_e.append(_eu)
					_l = len(_iu)
					if _l > _mlen:
						_mlen = _l
				# edit_pad_id is used as kb_pad_id
				_inpute, _kb, _edit = _inpute.new_tensor(pad_batch(_next_i, _mlen, pad_id=pad_id)), _kb.new_tensor(pad_batch(_next_k, _mlen, pad_id=edit_pad_id)), _edit.new_tensor(pad_batch(_next_e, _mlen, pad_id=edit_pad_id))
				_pad_mask = _inpute.eq(pad_id)
		if _rids:
			_mlm_mask = _inpute.eq(mask_id)
