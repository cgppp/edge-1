...
from transformer.GECToR.Encoder import Encoder as EncoderBase
...
class Encoder(EncoderBase):
	...
	def forward(self, inputs, kb=None, edit=None, token_types=None, mask=None, **kwargs):

		out = self.wemb(inputs)
		if kb is not None:
			out = out + self.kb_emb(kb)
		if edit is not None:
	...
	def build_task_model(self, *args, token_drop=token_drop, fix_init=True, **kwargs):
		...
		self.kb_emb = nn.Embedding(num_edit, self.wemb.weight.size(-1), padding_idx=pad_id)
	...
	def fix_task_init(self):
		...
		if hasattr(self, "edit_emb"):
			with torch_no_grad():
				...
				_ = 2.0 / sqrt(sum(self.kb_emb.weight.size()))
				self.kb_emb.weight.uniform_(-_, _)
				self.kb_emb.weight[pad_id].zero_()
				if self.temb is not None:
