...
from transformer.EqSparse.NMT import NMT
from utils.base import free_cache, fs_check, get_logger, mkdir, set_random_seed
...
if fine_tune_m is not None:
	...
	_conn_f = "%s.conn.h5" % fine_tune_m[:fine_tune_m.rfind("/")]
	if fs_check(_conn_f):
		mymodel.load_conn_from_file(_conn_f)
mymodel.save_conn_to_file(wkdir + "conn.h5")
