...
from utils.fmt.gec.kb.floader.noiser import Loader as DataLoader
...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	for seq_batch, seq_kb, seq_edt, seq_o in tqdm(td(), mininterval=tqdm_mininterval):
		...
		if mv_device:
			...
			seq_kb = seq_kb.to(mv_device, non_blocking=True)
			seq_edt = seq_edt.to(mv_device, non_blocking=True)
			seq_o = seq_o.to(mv_device, non_blocking=True)
		seq_batch, seq_kb, seq_edt, seq_o = seq_batch.to(torch.int64, non_blocking=True), seq_kb.to(torch.int64, non_blocking=True), seq_edt.to(torch.int64, non_blocking=True), seq_o.to(torch.int64, non_blocking=True)
		if mlm_enhance > 0.0:
			seq_batch, seq_kb, seq_edt, seq_o = get_batch(seq_batch, seq_kb, seq_edt, seq_o, p=mlm_enhance)

		with torch_autocast(enabled=_use_amp):
			loss = model(seq_batch, kb=seq_kb, edit=seq_edt, tgt=seq_o, prediction=False)[0]
			if multi_gpu:
		...
		loss = seq_batch = seq_kb = seq_edt = seq_o = None
		sum_loss += loss_add
		#if save_loss:
...
def eva(ed, nd, model, lossf, mv_device, multi_gpu, use_amp=False):
	...
	sum_loss = 0.0
	model.eval()
	src_grp, kb_grp, edt_grp, tgt_grp = ed["src"], ed["kb"], ed["edt"], ed["tgt"]
	with torch_inference_mode():
		for i in tqdm(range(nd), mininterval=tqdm_mininterval):
			bid = str(i)
			seq_batch = torch.from_numpy(src_grp[bid][()])
			seq_kb = torch.from_numpy(kb_grp[bid][()])
			...
			with torch_autocast(enabled=use_amp):
				loss, trans = model(seq_batch, kb=seq_kb, edit=seq_edt, tgt=seq_o, prediction=True)
				if multi_gpu:
			...
			r += correct.sum().item()
			correct = data_mask = trans = loss = seq_batch = seq_kb = seq_edt = seq_o = None
	w = float(w)
...
td = DataLoader(cnfg.train_data, vcbf=cnfg.plm_vcb, noise_char=cnfg.noise_char, noise_vcb=cnfg.noise_vcb, ngpu=len(cuda_devices) if multi_gpu else 1, print_func=None)#h5File(cnfg.train_data, "r", **h5_fileargs)
...
if (pre_trained_m is None) and (fine_tune_m is None):
