...
td = DataLoader(*cnfg.train_data, "%s%s/common.vcb" % (cnfg.cache_dir, cnfg.data_id,), ngpu=len(cuda_devices) if multi_gpu else 1, print_func=logger.info)#h5File(cnfg.train_data, "r", **h5_fileargs)
vd = td#h5File(cnfg.dev_data, "r", **h5_fileargs)
