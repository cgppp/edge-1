...
def train(td, tl, ed, nd, optm, lrsch, model, lossf, mv_device, logger, done_tokens, multi_gpu, multi_gpu_optimizer, tokens_optm=32768, nreport=None, save_every=None, chkpf=None, state_holder=None, statesf=None, num_checkpoint=1, cur_checkid=0, report_eva=True, remain_steps=None, save_loss=False, save_checkp_epoch=False, scaler=None):
	...
	global minerr, minloss, wkdir, save_auto_clean, namin, spt_ind
	model.train()
	...
	for i_d in tqdm(tl, mininterval=tqdm_mininterval):
		...
		seq_batch = torch.cat((spt_ind.expand(seq_batch.size(0), -1), seq_batch,), dim=1)
...
if cnfg.use_sep_emb:
	_nwordi_ext = nwordi + cnfg.num_spt
	spt_ind = torch.arange(nwordi, _nwordi_ext, dtype=torch.int, device=cuda_device).unsqueeze(0)
	nwordi = _nwordi_ext
	if cnfg.share_emb:
		nwordt = nwordi
else:
	spt_ind = torch.full((1, cnfg.num_spt,), sos_id, dtype=torch.int, device=cuda_device)
