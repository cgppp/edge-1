...
if cnfg.use_sep_emb:
	...
	spt_ind = torch.arange(nwordi, _nwordi_ext, dtype=torch.int).unsqueeze(0)
	nwordi = _nwordi_ext
	if cnfg.share_emb:
		nwordt = nwordi
else:
	spt_ind = torch.full((1, cnfg.num_spt,), sos_id, dtype=torch.int)
...
spt_ind = spt_ind.to(cuda_device, non_blocking=True)
...
with sys_open(sys.argv[1], "wb") as f, torch_inference_mode():
	for i in tqdm(range(ntest), mininterval=tqdm_mininterval):
		...
		seq_batch = torch.cat((spt_ind.expand(seq_batch.size(0), -1), seq_batch,), dim=1).to(torch.int64, non_blocking=True)
