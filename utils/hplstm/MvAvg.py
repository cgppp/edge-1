...
class MvAvgFunction(Function):
	...
	def forward(ctx, x, beta, inplace=False):
		...
		if x.size(1) > 1:
			if x.is_cuda:
				_ = x.size()
				_out = ml2mvavgs_cuda.forward(x, x if inplace else x.new_empty(_), beta, *_)
			else:
				_out = clmvavg_cpp.forward(x if inplace else x.clone(), 1, beta)
		else:
			mbeta = 1.0 - beta
			_out = x.mul_(mbeta) if inplace else x.mul(mbeta)
		ctx.beta = beta
	...
	def backward(ctx, grad_out):
		...
		if ctx.needs_input_grad[0]:
			if grad_out.size(1) > 1:
				if grad_out.is_cuda:
					_ = grad_out.size()
					grad_x = ml2mvavgs_cuda.backward(grad_out, grad_out.new_empty(_), ctx.beta, *_)
				else:
					grad_x = clmvavg_cpp.backward(grad_out.clone(), 1, ctx.beta)
			else:
