	...
	for (int i_bsize = block_id_x; i_bsize < bsize; i_bsize+=num_blocks_x) {
		for (int i_head = block_id_y; i_head < nhead; i_head+=num_blocks_y) {
			for (int i_isize = thread_id; i_isize < isize; i_isize+=num_threads) {
				scalar_t c = igh[i_bsize][0][i_head][i_isize] + init_cell[i_bsize][i_head][i_isize] * fgate[i_bsize][0][i_head][i_isize];
	...
	for (int i_bsize = block_id_x; i_bsize < bsize; i_bsize+=num_blocks_x) {
		for (int i_head = block_id_y; i_head < nhead; i_head+=num_blocks_y) {
			for (int i_isize = thread_id; i_isize < isize; i_isize+=num_threads) {
				...
				grad_fgate[i_bsize][0][i_head][i_isize] = agc * init_cell[i_bsize][i_head][i_isize];
			}
		}
	}
}
	...
	for (int i_bsize = block_id_x; i_bsize < bsize; i_bsize+=num_blocks_x) {
		for (int i_head = block_id_y; i_head < nhead; i_head+=num_blocks_y) {
			for (int i_isize = thread_id; i_isize < isize; i_isize+=num_threads) {
				...
				grad_fgate[i_bsize][0][i_head][i_isize] = gc * init_cell[i_bsize][i_head][i_isize];
			}
		}
	}
}
...
at::Tensor lgate_cuda_forward(torch::Tensor fgate, torch::Tensor igh, torch::Tensor init_cell, torch::Tensor cell, int bsize, int seqlen, int nhead, int isize) {
	...
	if (block_ul && grid_x_ul && grid_y_ul) {
		...
		AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, igh.scalar_type(), "lgate_aul_forward_cuda", ([&] {cuda_lgate_aul_<scalar_t><<<grid_size, block_size>>>(fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), seqlen);}));
	}
	else {
			if (!block_ul) {
			...
			AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, igh.scalar_type(), "lgate_forward_cuda", ([&] {cuda_lgate_<scalar_t><<<grid_size, block_size>>>(fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), bsize, seqlen, nhead, isize);}));
	}
...
std::vector<torch::Tensor> lgate_cuda_backward(torch::Tensor grad_cell, torch::Tensor cell, torch::Tensor fgate, torch::Tensor init_cell, torch::Tensor grad_fgate, torch::Tensor grad_igh, torch::Tensor grad_prev_cell, int bsize, int seqlen, int nhead, int isize) {
	...
	if (block_ul && grid_x_ul && grid_y_ul) {
		dim3 grid_size(bsize, nhead);
		if (use_li) {
			AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_cell.scalar_type(), "lgate_aul_li_backward_cuda", ([&] {cuda_lgate_aul_li_grad_<scalar_t><<<grid_size, block_size>>>(grad_cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), grad_fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_prev_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), seqlen - 1);}));
		}
		else {
			AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_cell.scalar_type(), "lgate_aul_nli_backward_cuda", ([&] {cuda_lgate_aul_nli_grad_<scalar_t><<<grid_size, block_size>>>(grad_cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), grad_fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_prev_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>());}));
		}
	}
	else {
		if (!block_ul) {
		...
		if (use_li) {
			AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_cell.scalar_type(), "lgate_li_backward_cuda", ([&] {cuda_lgate_li_grad_<scalar_t><<<grid_size, block_size>>>(grad_cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), grad_fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_prev_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), bsize, nhead, isize, seqlen - 1);}));
		}
		else {
			AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, grad_cell.scalar_type(), "lgate_nli_backward_cuda", ([&] {cuda_lgate_nli_grad_<scalar_t><<<grid_size, block_size>>>(grad_cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), cell.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), init_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), grad_fgate.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_igh.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(), grad_prev_cell.packed_accessor32<scalar_t, 3, torch::RestrictPtrTraits>(), bsize, nhead, isize);}));
		}
	}
