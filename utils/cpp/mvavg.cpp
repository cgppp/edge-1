...
at::Tensor mvavg_forward(torch::Tensor x, int64_t dim, float beta=0.9, bool inplace=false) {
	...
	auto prev_step = out.select(dim, 0).mul_(mbeta);
	auto seqlen = x.size(dim);
	int64_t i;
	for (i = 1; i < seqlen; i++) {
		prev_step = out.select(dim, i).mul_(mbeta).add_(prev_step, beta);
	}
