...
def handle(srcf, tgtf, rssf, rskf, rsef, rstf, **kwargs):

	ens = "\n".encode("utf-8")
	with sys_open(srcf, "rb") as frds, sys_open(tgtf, "rb") as frdt, sys_open(rssf, "wb") as fwrts, sys_open(rskf, "wb") as fwrtk, sys_open(rsef, "wb") as fwrte, sys_open(rstf, "wb") as fwrtt:
		for s, t in zip(frds, frdt):
			...
			if _s and _t:
				...
				for _src, _kb, _edit, _tgt in generate_iter_data(_s, None, _t):
