...
def handle(fsrc, vcb, frs, template="instruct", system="You are a helpful assistant.", templated=templated):
	...
	_ = (lambda lin, processor: " ".join(iter_to_str(processor(_tfunc(system, lin))[2:]))) if template == "assistant" else (lambda lin, processor: " ".join(iter_to_str(processor(_tfunc(system, lin)))))
