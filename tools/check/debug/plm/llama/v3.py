...
llm_path = "plm/Llama-3.2-1B-Instruct"

disu = lambda a, b: (a.squeeze() - b.squeeze()).abs()
disf = lambda a, b: disu(a, b).sum()

print("Tokenization")
ht = transformers.PreTrainedTokenizerFast.from_pretrained(llm_path)#LlamaTokenizerFast
...
hm = transformers.LlamaForCausalLM.from_pretrained(llm_path)
hm.eval()
