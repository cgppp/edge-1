...
class LayerNorm(LayerNormBase):
	...
	def forward(self, x, **kwargs):
		...
		out = layer_norm(x.float(), self.normalized_shape, None, None, self.eps).to(x.dtype, non_blocking=True)
...
class RMSNorm(RMSNormBase):
	...
	def forward(self, x, **kwargs):
		...
		out = rms_norm(x.float(), self.normalized_shape, None, self.eps).to(x.dtype, non_blocking=True)
		if self.weight is not None:
