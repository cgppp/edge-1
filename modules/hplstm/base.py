...
class HPLSTM(nn.Module):
	...
	def __init__(self, isize, num_head=8, osize=None, act_drop=0.0, enable_proj_bias=enable_proj_bias_default, MHPLSTMCore=MHPLSTMCore, i_hsize=None, o_hsize=None, **kwargs):

		super(HPLSTM, self).__init__()

		_osize = parse_none(osize, isize)
		_o_hsize = num_head * float2odd(float(parse_none(o_hsize, _osize)) / num_head)

		self.head_dim = float2odd(float(parse_none(i_hsize, isize)) / num_head)
		_i_hsize = num_head * self.head_dim
		self.num_head = num_head

		self.trans_input = Linear(isize, _i_hsize, bias=enable_proj_bias)
		self.net = MHPLSTMCore(_i_hsize, num_head=self.num_head, osize=_o_hsize, act_drop=act_drop, **kwargs)
		self.trans_output = Linear(_o_hsize, _osize, bias=enable_proj_bias)
...
class BiHPLSTM(nn.Module):
	...
	def __init__(self, isize, num_head=8, osize=None, act_drop=0.0, enable_proj_bias=enable_proj_bias_default, MHPLSTMCore=MHPLSTMCore, i_hsize=None, o_hsize=None, **kwargs):
		...
		_i, _o = _i_hsize + _i_hsize, _o_hsize + _o_hsize
		self.trans_input = Linear(isize, _i, bias=enable_proj_bias)
		self.net = MHPLSTMCore(_i, num_head=self.num_head + self.num_head, osize=_o, act_drop=act_drop, **kwargs)
		self.trans_output = Linear(_o, _osize, bias=enable_proj_bias)
