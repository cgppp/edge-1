...
class MHPLSTMCore(nn.Module):
	...
	def __init__(self, isize, num_head=8, osize=None, fhsize=None, dropout=0.0, act_drop=None, custom_act=use_adv_act_default, enable_bias=enable_proj_bias_default, use_glu=use_glu_ffn, **kwargs):
		...
		_drop_ind = 2
		if use_glu is None:
			_.extend([Custom_Act() if custom_act else nn.ReLU(inplace=True), GroupLinearCore(_fhsize, o_hsize, num_head, bias=enable_bias)])
		else:
			_.append(get_act(use_glu)())
			_.append(GroupLinearCore(_fhsize // 2, o_hsize, num_head, bias=enable_bias))
		if dropout > 0.0:
		...
		self.trans_ifg = GroupLinear(i_hsize + i_hsize, o_hsize + o_hsize, num_head, bias=True, shuffle=False, trans_input=False, flatten_output=False)
