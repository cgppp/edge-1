...
class MHPLSTMCore(MHPLSTMCoreBase):
	...
	def forward(self, heads_input, states=None, head_mask=None, **kwargs):
		...
		if states is None:
			...
			csum = torch.cat((heads_input.new_zeros(bsize, 1, nheads, adim) if _init_state else states[0], heads_input.detach(),), dim=1).cumsum(dim=1)
		...
		return out if states is None else (out, (csum_state_return, cell.select(1, seql - 1).detach(),),)
