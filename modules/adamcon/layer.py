...
class PositionwiseFF(PositionwiseFFBase):
	...
	def forward(self, x, **kwargs):
		...
		out = self.adamcon(_out if self.norm_residual else x, out)
...
class ResSelfAttn(ResSelfAttnBase):
	...
	def forward(self, iQ, *inputs, **kwargs):
		...
		if isinstance(outs, tuple):
			...
			return self.adamcon(_iQ if self.norm_residual else iQ, _out), *outs[1:]

		else:
			if self.drop is not None:
				outs = self.drop(outs)

			return self.adamcon(_iQ if self.norm_residual else iQ, outs)
