...
class SigmoidITensor(nn.Module):
	...
	def __init__(self, warm_steps, target_value, xseql=cache_len_default, **kwargs):
		...
		self.tarv = target_value
		self.xseql = xseql
		self.register_buffer("w", ((((torch.arange(1, xseql + 1, dtype=torch.float32, requires_grad=False) / self.wstep)).sigmoid() * 2 - 1) * self.tarv).unsqueeze(0).unsqueeze(-1), persistent=False)
